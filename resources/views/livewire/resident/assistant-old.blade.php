<div x-data="assistantChatNew()" x-init="init()">
    <script>
        function assistantChatNew() {
            return {
                ...
                async send() {
                    ...
                    const msg = this.$wire.messages[idx - 1]?.text || '';
                    await this.$wire.set('messages.' + idx + '.text', '');

                    console.log('Sending message:', msg);

                    try {
                        const token = this.$wire.token || '';
                        const timezone = this.timezone();
                        
                        const res = await fetch('/api/assistant/chat?t=' + Date.now(), {
                            method: 'POST',
                            headers: {
                            ...
                            for (let line of lines) {
                                ...
                                try {
                                    ...
                                    if (data?.type === 'token') {
                                        ...
                                        const current = this.$wire.messages[idx].text;
                                        await this.$wire.set('messages.' + idx + '.text', current + content);
                                    ...
                                    await this.$wire.set('messages.' + idx + '.text', 'Error: ' + e.message);
                                }
                            }
                        }
                    } catch (err) {
                        await this.$wire.set('messages.' + idx + '.text', 'Error: ' + err.message);
                    }
...
<div class="flex flex-col gap-3 rounded-2xl border border-slate-200/70 bg-white/95 p-4 shadow-sm transition-colors duration-300 dark:border-slate-700/60 dark:bg-slate-900/70">
    ...
    <div class="flex flex-col gap-3 max-h-56 overflow-y-auto pr-1">
        @foreach ($messages as $message)
            <div class="{{ $message['type'] === 'assistant' ? 'ml-0 mr-auto rounded-2xl rounded-bl-sm bg-sky-50 px-3 py-2 text-xs font-medium text-slate-700 dark:bg-sky-500/10 dark:text-sky-100' : 'ml-auto mr-0 rounded-2xl rounded-br-sm bg-slate-900 px-3 py-2 text-xs font-semibold text-white dark:bg-slate-700' }}">
        ...
        <div class="flex flex-col gap-2">
            ...
            <div class="flex flex-wrap gap-2">
                @foreach ($suggestions as $suggestion)
                    <button type="button" wire:click="useSuggestion('{{ $suggestion }}')" class="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1 text-[11px] font-medium text-slate-500 transition-colors duration-200 hover:border-sky-200 hover:text-sky-700 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-400 dark:hover:border-sky-500 dark:hover:text-sky-200">
                        {{ $suggestion }}
                    </button>
                @endforeach
            </div>
