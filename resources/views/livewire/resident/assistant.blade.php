...
function kbFeedbackWidget(meta = {}) {
    ...
    <script>
        function interactionFeedbackWidget(meta = {}) {
            return {
                state: 'ask',
                note: '',
                submitting: false,
                ...
                async submit(helpful) {
                    if (this.submitting || this.submitted || !this.meta.interaction_id) {
                        return;
                    }

                    if (!helpful && this.state === 'ask') {
                        this.startNegativeFlow();
                        return;
                    }
                    ...
                    try {
                        const res = await fetch('/api/assistant/feedback/interactions', {
                            method: 'POST',
                            headers,
                            body: JSON.stringify({
                                interaction_id: this.meta.interaction_id,
                                helpful,
                                reason: helpful ? null : (this.note || null),
                            }),
                        });
        ...
        @foreach ($messages as $message)
            <div class="message-bubble {{ $message['type'] === 'assistant' ? 'ml-0 mr-auto' : 'ml-auto mr-0' }}" wire:key="chat-message-{{ $loop->index }}">
                @if($message['type'] === 'assistant')
                    ...
                    @endif
                    @if (!empty($message['meta']['interaction_id']) && empty($message['meta']['knowledge_feedback']['token'] ?? null))
                        <div class="ml-9 mt-2" x-data="interactionFeedbackWidget(@js($message['meta']))" x-cloak>
                            <div class="rounded-2xl border border-slate-200/70 bg-white/90 px-4 py-3 text-xs text-slate-600 shadow-sm dark:border-slate-700 dark:bg-slate-800/70 dark:text-slate-200" x-show="state === 'ask'">
                                <p class="font-semibold text-slate-700 dark:text-slate-100">Apakah respon ini sudah membantu?</p>
                                <div class="mt-2 flex gap-2">
                                    <button type="button" class="rounded-full bg-emerald-500/90 px-3 py-1 font-semibold text-white shadow-sm transition hover:bg-emerald-600 disabled:opacity-60" @click="submit(true)" :disabled="submitting">Sudah pas</button>
                                    <button type="button" class="rounded-full border border-slate-300 px-3 py-1 font-semibold text-slate-600 transition hover:border-rose-400 hover:text-rose-500 dark:border-slate-600 dark:text-slate-200" @click="submit(false)" :disabled="submitting">Kurang tepat</button>
                                </div>
                            </div>
                            <div class="rounded-2xl border border-rose-200/80 bg-rose-50/70 px-4 py-3 text-xs text-rose-700 shadow-sm dark:border-rose-700/40 dark:bg-rose-900/40 dark:text-rose-100" x-show="state === 'note'">
                                <p class="font-semibold">Bagian mana yang perlu diperbaiki?</p>
                                <textarea x-model="note" rows="2" class="mt-2 w-full rounded-xl border border-rose-200 bg-white/90 px-3 py-2 text-slate-700 focus:border-rose-400 focus:outline-none dark:border-rose-700/70 dark:bg-rose-900/30 dark:text-slate-100" placeholder="Contoh: tolong detailkan dokumen yang dibutuhkan"></textarea>
                                <div class="mt-3 flex gap-2">
                                    <button type="button" class="rounded-full bg-rose-500 px-3 py-1 font-semibold text-white shadow-sm transition hover:bg-rose-600 disabled:opacity-60" @click="submit(false)" :disabled="submitting">Kirim</button>
                                    <button type="button" class="rounded-full border border-rose-200/80 px-3 py-1 font-semibold text-rose-600 transition hover:border-rose-300 hover:text-rose-700 dark:border-rose-700/70 dark:text-rose-200" @click="state = 'ask'; note='';" :disabled="submitting">Batal</button>
                                </div>
                            </div>
                            <div class="rounded-2xl border border-emerald-200/80 bg-emerald-50/70 px-4 py-3 text-xs font-semibold text-emerald-700 dark:border-emerald-700/40 dark:bg-emerald-900/30 dark:text-emerald-200" x-show="state === 'done'">
                                Terima kasih! Masukan kamu sudah dicatat.
                            </div>
                            <div class="rounded-2xl border border-amber-200/80 bg-amber-50/70 px-4 py-3 text-xs font-semibold text-amber-700 dark:border-amber-700/40 dark:bg-amber-900/30 dark:text-amber-200" x-show="state === 'error'">
                                <span x-text="error"></span>
                            </div>
                        </div>
                    @endif
