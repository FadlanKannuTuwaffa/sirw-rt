...
class SendReminderTelegram implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        if (! $reminder) {
            $this->logger()->warning('Reminder tidak ditemukan saat mengirim Telegram', [
        ...
        if (! $model instanceof Bill && ! $model instanceof Event) {
            $this->markChannelFailed($reminder->id, 'telegram', 'Model pengingat tidak valid');
            return;
        }

        $recipients = $this->resolveTelegramRecipients($model);

        if ($recipients->isEmpty()) {
            $this->markChannelSkipped($reminder->id, 'telegram', 'Tidak ada akun Telegram aktif');
        ...
        try {
            foreach ($recipients as $user) {
                ...
                $acc = $user->telegramAccount;

                if (! $acc || ! $acc->isActive || ! $acc->receive_notifications) {
                    continue;
                }

                $chatId = $acc->telegram_user_id;
            ...
            $this->markChannelFailed($reminder->id, 'telegram', $exception->getMessage());
            $this->logger()->error('Gagal mengirim Telegram reminder', [
        ...
        $this->updateChannelState($reminder->id, 'telegram', [
            'status' => 'sent',
            'sent_at' => Carbon::now()->toDateTimeString(),
            'recipients' => array_filter($chatIds),
        ]);

        $this->logger()->info('Telegram reminder berhasil dikirim', [
            'reminder_id' => $reminder->id,
            'recipients' => array_filter($chatIds),
        ]);
    ...
    private function resolveTelegramRecipients(Bill|Event $model): Collection
    {
        ...
        return $recipients
            ->filter(function (User $user) {
                ...
                return $account
                    && is_null($account->unlinked_at)
                    && $account->receive_notifications
                    && filled($account->telegram_chat_id);
            })
            ->unique(function (User $user) {
                return optional($user->telegramAccount)->telegram_chat_id;
            });
    }
    ...
    private function markChannelSkipped(int $reminderId, string $channel, string $reason): void
    {
        ...
        $this->logger()->notice('Kanal Telegram dilewati', [
    ...
    private function logger(): LoggerInterface
    {
        ...
        if ($logger === null) {
            $logger = Log::build([
                'driver' => 'single',
                'path' => storage_path('logs/reminder_telegram.log'),
            ]);
        }
